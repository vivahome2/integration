...
class WPSC_Google_Analytics {
    ...
    add_action( 'wpsc_transaction_results_shutdown', array( $this, 'add_transaction_datalayer' ), 10, 1 );
	}
	...
	public function add_transaction_datalayer( $session_id ) {
		global $wpdb;
		...
		$output .= "'transactionId': '" . $purchase_id . "',"          // order ID - required
			. "'transactionAffiliation': '" . wp_specialchars_decode( $this->get_site_name() ) . "'," // affiliation or store name
			. "'transactionTotal': " . wpsc_currency_display( $purchase->totalprice ) . "',"   // total - required
			. "'transactionTax': " . wpsc_currency_display( $total_tax ) . "',"              // tax
			. "'transactionShipping': " . wpsc_currency_display( $total_shipping ) . "',"         // shipping
			. "'transactionCity': " . wp_specialchars_decode( $city ) . "',"                  // city
			. "'transactionState': " . wp_specialchars_decode( $state ) . "',"                 // state or province
			. "'transactionCountry': " . wp_specialchars_decode( $country ) . "'"                // country
			. ",'transactionProducts': [\n\r";
